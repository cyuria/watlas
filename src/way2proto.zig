...
pub const Client = struct {
    ...
    pub fn send(
        ...
        const data = try std.mem.concat(alloc, u8, &.{
        ...
        log.debug("Sending Packet [{}]:{x}", .{
            data.len,
            std.mem.bytesAsSlice(u32, data),
        });
        const length = try self.socket.?.write(data);
        std.debug.assert(length == data.len);
    }
